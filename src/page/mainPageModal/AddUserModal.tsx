...
export default function AddUserModal({ isModalOpen, closeModal }: ModalProps) {
  ...
  const handleAddUser = () => {
    ...
    if (nickname === "example" && tag === "KR1234") {
        ...
        >
          ...
          <div className="flex justify-center gap-4 mt-10">
            <button
              ...
              className={`px-7 py-2 ${
                isAddEnabled
                  ? "bg-[#C89B3C] text-white hover:bg-[#A87F2D]"
            ...
            <button
              onClick={closeModal}
              className="px-7 py-2 bg-[#F0E6D2] text-black border-2 border-[#C89B3C] rounded-full hover:bg-[#e8d9c3]"
            >
              닫기
            </button>
          </div>
